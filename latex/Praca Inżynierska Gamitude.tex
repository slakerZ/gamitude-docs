...
\subsection{Plik główny App.tsx}
App składa się z drawer'ów po lewej i prawej stronie oraz okna wyświetlacego strony po środku.
 Po lewej znajduje się nawigacja, po prawej panel kontrolny.
\begin{figure}
	\centering
	\ContinuedFloat
	\includegraphics[width=\textwidth]{implementacja/frontend/app}\\
	\caption{Kod App.tsx}
	\label{fig:app}
\end{figure}

pod każdą subsection diagram komponentów dla danej funkcjonalności
...
Komponent Bullet Journal Page został rozbity na kilka pomniejszych kompotentów.
 ...
 BulletTask zawiera w sobie komponent TaskTile,
 którego zadaniem jest wyrenderowanie zawartości każdego zadania tj. możliwość edycji zadania, możliwość usunięcia zadania, czy możliwość jego zakńczenia.\\
 Komponenty AddJournalDialog i AddPageDialog są to komponenty typu modal,
...
\subsubsection{Page}
\subsubsection{BulletTask}
\subsubsection{TaskTile}
\subsubsection{AddJournalDialog}
\subsubsection{AddPageDialog}
\subsubsection{AddProjectTaskDialog}
...
Pozostałe 2 zostały stwrzone na potrzeby testowania.
\begin{figure}[H]
	\centering
...
Projekt zawiera również folder Utils, w którym przechowywane są funkcje pomocnicze.
\subsection{Utills}
Klasy extensions rysunek nr \ref{fig:gamitude_extension} oraz  middleware rysunek nr \ref{fig:gamitude_middleware}.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{implementacja/backend/przykład_extension}\\
	\caption{Przykład extension}
	\label{fig:gamitude_extension}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{implementacja/backend/przykład_middleware}\\
	\caption{Przykład middleware}
	\label{fig:gamitude_middleware}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{implementacja/backend/register_services_startup}\\
	\caption{Rejestracja serwisów w startup.cs}
	\label{fig:gamitude_services_startup}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{implementacja/backend/przykład_pipeline_startup}\\
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{implementacja/backend/przykład_get}\\
	\caption{Metoda GET w kontrolerze}
	\label{fig:przykład_get}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{implementacja/backend/przykład_post}\\
	\caption{Metoda POST w kontrolerze}
	\label{fig:przykład_post}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{implementacja/backend/przykład_put}\\
	\caption{Metoda PUT w kontrolerze}
	\label{fig:przykład_put}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{implementacja/backend/przykład_delete}\\
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{implementacja/backend/przykład_timer_repo}\\
